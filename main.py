...
def get_tiles_from_txt(file_path: str):
...
EXCLUDED = get_tiles_from_txt('/DATA/S2/PRODUCTS/GEOFLOW/tiles_exclude.txt')

CROZET = get_tiles_from_txt('/DATA/S2/PRODUCTS/GEOFLOW/tiles_crozet.txt')
EPARSES = get_tiles_from_txt('/DATA/S2/PRODUCTS/GEOFLOW/tiles_eparses.txt')
KERGUELEN = get_tiles_from_txt('/DATA/S2/PRODUCTS/GEOFLOW/tiles_kerguelen.txt')
MADAGASCAR = get_tiles_from_txt('/DATA/S2/PRODUCTS/GEOFLOW/tiles_mada.txt')
MAURICE = get_tiles_from_txt('/DATA/S2/PRODUCTS/GEOFLOW/tiles_maurice.txt')
REUNION = get_tiles_from_txt('/DATA/S2/PRODUCTS/GEOFLOW/tiles_reunion.txt')
RODRIGUES = get_tiles_from_txt('/DATA/S2/PRODUCTS/GEOFLOW/tiles_rodrigues.txt')
SEYCHELLES = get_tiles_from_txt('/DATA/S2/PRODUCTS/GEOFLOW/tiles_seychelles.txt')
TROMELIN = get_tiles_from_txt('/DATA/S2/PRODUCTS/GEOFLOW/tiles_tromelin.txt')
...
def shp_tuile(tile_name):
    try:
        get_processed_tile_vect('/DATA/S2/PRODUCTS/GEOFLOW/data/shp', tile_name)
...
def create_netcdf(indice_name: str, tile_name : str, start_year: int, path_to_create: str, nc_file_name: str, y_size: int = 0, x_size: int = 0, times_size: int = 0, crs_code:str = '', wkt:str = ''):
    ...
    geoflow_indice_md_df = pandas.read_csv('/DATA/S2/PRODUCTS/metadata/METADATA_SEN2CHAIN_indices.csv', header=0, usecols=lambda col: col not in ['Data'])
...
def df_to_csv(df, tile: str):
    ...
    df.to_csv('/DATA/S2/PRODUCTS/GEOFLOW/metadata/METADATA_'+tile+'.csv', index=False)
    print('/DATA/S2/PRODUCTS/GEOFLOW/metadata/METADATA_'+tile+'.csv created')
...
pour la valorisation et l'accès aux données issues
    ...
    while (True):
        ...
        if option == 4:
            ...
            dest_path = '/DATA/S2/PRODUCTS/GEOFLOW/data/nc'

            region = get_tiles_from_menu()
            sen2chain_to_netcdf(src_path, list_indices, region, dest_path)
            ...
            if option != '0':
                region = get_tiles_from_menu()
                for t in region:
                    src_path = '/DATA/S2/PRODUCTS/INDICES/' + indice + '/' + t
                    dest_path = '/DATA/S2/PRODUCTS/GEOFLOW/data/zip/'
                    for year in temporal_coverage:
                        zip_dir(src_path, dest_path + indice + '_' + t + '_' + year + '.zip', '*' + year + '*')

        elif option == 7:
            region = get_tiles_from_menu()
            for t in region:
                zenodo_upload('DATA/S2/PRODUCTS/GEOFLOW/metadata/METADATA_' + t + '.csv')
